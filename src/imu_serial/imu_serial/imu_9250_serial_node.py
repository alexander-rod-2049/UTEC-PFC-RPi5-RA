...
class Imu9250Publisher(Node):

    def __init__(self):
        super().__init__('imu_9250_publisher')
        ...
        try:
            ...
            if raw_data:
                ...
                if decoded_data.startswith('{') and decoded_data.endswith('}'):
                    ...
                    yaw_deg = imu_data.get('y', 0.0)
                    # Convertir a radianes (ROS usa radianes)
                    ...
                    msg.orientation_covariance[0] = 0.25
                    
                    # Datos del magnetómetro
                    mag_msg = MagneticField()
                    mag_msg.header.stamp = time_now
                    mag_msg.header.frame_id = "imu_link"
                    mag_msg.magnetic_field.x = imu_data.get('mx', 0.0) * 1e-6 # Convertir uT a Teslas
                    ...
                    t.transform.rotation = msg.orientation  # SOLO yaw importa visualmente
...
def main(args=None):
    ...
    imu9250_publisher = Imu9250Publisher()

    rclpy.spin(imu9250_publisher)

    # Destroy the node explicitly
    # (optional - otherwise it will be done automatically
    # when the garbage collector destroys the node object)
    imu9250_publisher.destroy_node()
    rclpy.shutdown()
